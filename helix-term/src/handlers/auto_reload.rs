...
fn handle_document_change(
    ...
    if doc.is_modified() {
        ...
        match doc.reload(view, &editor.diff_providers, true) {
            Ok(_) => {
...
fn reload_vcs_diffs(editor: &mut Editor) {
    for doc in editor.documents.values_mut() {
        ...
        match editor.diff_providers.get_diff_base(path, true) {
        ...
        move |cx, _input, event| {
            match event {
                PromptEvent::Validate => {
                    ...
                    match doc.reload(view, &cx.editor.diff_providers, true) {
                        Ok(_) => {
                            view.ensure_cursor_in_view(doc, scrolloff);
